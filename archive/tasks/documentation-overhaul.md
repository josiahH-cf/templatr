...
- **Caveats from audit (resolved):** The `platform-config-consolidation` spec is now complete. Template seeding runs on first launch, so `pip install` now gives a working out-of-box experience. The phantom PowerShell installer reference has been removed from `install.sh`. Windows users should use pre-built binaries from GitHub Releases or WSL2.
- **Status:** [x] Complete
...
- **Caveats from audit (resolved):** The `platform-config-consolidation` spec is now complete. `pip install -e .` now seeds templates on first launch — the empty-state gap is fixed. Error messages now use platform-correct paths. The phantom PowerShell installer reference has been removed from `install.sh`.
- **Status:** [x] Complete
