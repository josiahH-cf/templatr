...
class TestTemplatesMdExists:
    ...
    def test_file_exists(self):
        """TEMPLATES.md exists at the project root."""
        doc = Path(__file__).resolve().parent.parent / "TEMPLATES.md"
        assert doc.exists(), "TEMPLATES.md not found at project root"

    def test_contains_new_command(self):
        """TEMPLATES.md mentions the /new command."""
        doc = Path(__file__).resolve().parent.parent / "TEMPLATES.md"
        text = doc.read_text(encoding="utf-8")
        assert "/new" in text

    def test_contains_variable_syntax(self):
        """TEMPLATES.md documents the {{variable}} placeholder syntax."""
        doc = Path(__file__).resolve().parent.parent / "TEMPLATES.md"
        text = doc.read_text(encoding="utf-8")
        assert "{{" in text

    def test_contains_advanced_edit(self):
        """TEMPLATES.md mentions the Advanced Edit option."""
        doc = Path(__file__).resolve().parent.parent / "TEMPLATES.md"
        text = doc.read_text(encoding="utf-8")
        assert "Advanced Edit" in text

    def test_contains_import_export(self):
        """TEMPLATES.md documents import and export functionality."""
        doc = Path(__file__).resolve().parent.parent / "TEMPLATES.md"
        text = doc.read_text(encoding="utf-8")
    ...
    def test_contains_three_steps(self):
        """TEMPLATES.md contains a numbered 3-step workflow."""
        doc = Path(__file__).resolve().parent.parent / "TEMPLATES.md"
        text = doc.read_text(encoding="utf-8")
